...
export function getProductsInStock(products) {
    return products.filter((product) => {
        return product.inStock ? true : false;
    });
}
export function getProductsOutOfStock(products) {
    return products.filter((product) => {
        return !product.inStock ? true : false;
    });
}
...
export function printProducts(products) {
    products.map((product) => {
        product.inStock
            ? console.log(product.title + " - " + formatPrice(product.price) + " (в наличии)")
